# Scenario: You are a Data Sceintist working for a consultin firm
# One of your colleagues from the auditing department has asked you to help them assess the financial statement of organization X

# You have been supplied with two vectors of data:
# Monthly revenue and monthly expenses for the financial year in question. Your task is to caclculate the following financial metrics:
# Profit for each month
# Profit after tax for each month (the tax rate is 30%)
# profit margin for each month - equals to profit after tax divided by revenue
# good months - where the profit after tax was greater than the mean for the year
# bad months - where the profit after tax was less than the mean for the year
# The best month - where the profit after tax was masx for the year
# The worst month - wherer the profit after tax was min for the year.

# All results need to be presented as vectors.
# Results for dollar values need to be calculated with $0.01 precision, but need to be presented in Units of $1,000(i.e., 1K) with no decimal points. 

# Results for the profit margin ratio need to be presented in units of % with no decimal points. 

# Note: Your colleague has warned you that is is okay for tax for any given month to be negative (in accounting terms, negative tax translates into a deferred tax asset).

#Data
revenue <- c(14574.49, 7606.46, 8611.41, 9175.41, 8058.65, 8105.44, 11496.28, 9766.09, 10305.32, 14379.96, 10713.97, 15433.50)
expenses <- c(12051.82, 5695.07, 12319.20, 12089.72, 8658.57, 840.20, 3285.73, 5821.12, 6976.93, 16618.61, 10054.37, 3803.96)

# Solution

# Profit for each month
profitPerMonth <- revenue - expenses

# Profit after tax for each month (the tax rate is 30%)
tax <- 0.30
afterTaxProfit <- (revenue * (1 - tax)) - expenses

# profit margin for each month - equals to profit after tax divided by revenue
profitmargin = (afterTaxProfit / revenue) * 100
print(paste(round(profitmargin,2), '%'))
months <- rep(0,length(revenue))
counter <- 0
for(i in afterTaxProfit){
  counter <- counter + 1
  if(i > mean(afterTaxProfit)){
    # good months - where the profit after tax was greater than the mean for the year
    months[counter] <- 1
    # bad months - where the profit after tax was less than the mean for the year
    }
  else {
    months[counter] <- 0
    }
}

# The best month - where the profit after tax was masx for the year
bestMonth = max(afterTaxProfit[months])

# The worst month - wherer the profit after tax was min for the year.
worstMonth = min(afterTaxProfit[!months])
